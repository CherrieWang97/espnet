...
from espnet.nets.pytorch_backend.casecade import E2E
import matplotlib
matplotlib.use('Agg')
...
class ASRConverter(object):
    ...
    def __call__(self, batch, device, src_id, trg_id):
        ...
        src = [np.concatenate([[src_id], y[1]]) for y in ys]
        trg = [np.concatenate([[trg_id], y[0]]) for y in ys]
        # NOTE: this is for multi-task learning (e.g., speech translation)
        src_pad = pad_list([torch.from_numpy(np.array(sent)).long() for sent in src], self.ignore_id).to(device)
        trg_pad = pad_list([torch.from_numpy(np.array(sent)).long() for sent in trg], self.ignore_id).to(device)
        
        return xs_pad, ilens, src_pad, trg_pad
...
class CustomEvaluator(extensions.Evaluator):
    ...
    def __init__(self, model, iterator, target, converter, device, src_id=10000, trg_id=10001):
...
class CustomUpdater(training.StandardUpdater):
    ...
    def __init__(self, model, grad_clip_threshold, train_iter,
                 optimizer, converter, device, ngpu, src_id=10000, trg_id=10001):
...
def train(args):
    ...
    updater = CustomUpdater(
        model, args.grad_clip, train_iter, optimizer, converter, device, args.ngpu, args.src_id, args.trg_id)
