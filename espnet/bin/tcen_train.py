...
def get_parser(parser=None, required=True):
    ...
    if parser is None:
        parser = configargparse.ArgumentParser(
            description="Train a TCEN model on one CPU, one or multiple GPUs",
    ...
    # task related
    parser.add_argument('--st-train-json', type=str, default=None,
                        help='Filename of st train label data (json)')
    parser.add_argument('--asr-train-json', type=str, default=None,
                        help='Filename of asr train label data (json)')
    parser.add_argument('--mt-train-json', type=str, default=None,
                        help='Filename of mt train label data (json)')
    ...
    parser.add_argument('--model-module', type=str, default=None,
                        help='model defined module (default: espnet.nets.xxx_backend.e2e_tcen:E2E)')
    ...
    parser.add_argument('--mt-batch-size', '--mt-batch-seqs', '-mb', default=0, type=int,
                        help='Maximum seqs in a minibatch (0 to disable)')
    ...
    parser.add_argument('--mt-maxlen-in', '--mt-batch-seq-maxlen-in', default=100, type=int, metavar='ML',
                        help='When --batch-count=seq, batch size is reduced if the input sequence length > ML.')
    parser.add_argument('--mt-maxlen-out', '--mt-batch-seq-maxlen-out', default=100, type=int, metavar='ML',
                        help='When --batch-count=seq, batch size is reduced if the output sequence length > ML')
    ...
    parser.add_argument('--asr-model', default=None, type=str, nargs='?',
                        help='Pre-trained ASR model to initialize speech encoder.')
    parser.add_argument('--mt-model', default=None, type=str, nargs='?',
                        help='Pre-trained MT model to initialize decoder.')
...
def main(cmd_args):
    ...
    if args.backend == "pytorch":
        from espnet.st.pytorch_backend.st_tcen import train
        train(args)
    else:
