...
class E2E(STInterface, torch.nn.Module):
    ...
    def __init__(self, idim, odim, args, ignore_id=0, asr_model=None, mt_model=None):
        ...
        self.pooler = torch.nn.Linear(self.xlm.config.hidden_size, 2)
    ...
    def forward(self, xs_pad, ilens, ys_pad, transcript_label=None, task='tp'):
        ...
        if task == "tp":
            ...
            cls_token = torch.ones([sp_hs_pad.size(0), 1]).long().to(sp_hs_pad.device)
            cls_embedding = self.xlm.embeddings(cls_token)
            ...
            sp_lang_ids = torch.zeros_like(sp_position_ids)
            #sp_position_embeddings = self.bert.bert.embeddings.position_embeddings(sp_position_ids)
            sp_lang_embeddings = self.xlm.lang_embeddings(sp_lang_ids)
            sp_hs_pad = sp_hs_pad + sp_lang_embeddings
            ...
            sp_bert_mask = self.xlm.get_masks(lens, sp_hs_pad.size(1), False, PADDING_MASK)
