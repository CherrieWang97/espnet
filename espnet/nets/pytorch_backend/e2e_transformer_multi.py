...
class E2E(ASRInterface, torch.nn.Module):
    ...
    def __init__(self, idim, odim, mdim, args, ignore_id=-1, asr_model=None, mt_model=None):
        ...
        if asr_model:
            ...
            for n, p in self.named_parameters():
                asr_n = n.replace('s_encoder.', 'encoder.')
                asr_n = asr_n.replace('s_decoder.', 'decoder.')
        ...
        if mt_model:
            ...
            for n, p in self.named_parameters():
                mt_n = n.replace('t_encoder.', 'encoder.')
                mt_n = mt_n.replace('t_decoder.', 'decoder.')
    ...
    def forward(self, xs_pad, ilens, ys_pad, task='st'):
        ...
        if task == "st" or task == "mt":
            ...
            pred_pad, pred_mask = self.t_decoder(ys_in_pad, ys_mask, hs_pad, hs_mask)
            loss = self.criterion(pred_pad, ys_out_pad)
            acc = th_accuracy(pred_pad.view(-1, self.odim), ys_out_pad,
                              ignore_label=self.ignore_id)
        else:
            ys_in_pad, ys_out_pad = add_sos_eos(ys_pad, self.mdim-1, self.mdim-1, self.ignore_id)
            ys_mask = target_mask(ys_in_pad, self.ignore_id)
            pred_pad, pred_mask = self.s_decoder(ys_in_pad, ys_mask, hs_pad, hs_mask)
            loss = self.asr_criterion(pred_pad, ys_out_pad)
            acc = th_accuracy(pred_pad.view(-1, self.mdim), ys_out_pad,
                              ignore_label=self.ignore_id)
        ...
        mt_acc = None
        if task == "st":
            loss_st_data = float(loss)
            st_acc = acc
        if task == "asr":
            loss_asr_data = float(loss)
            asr_acc = acc
        if task == "mt":
            loss_mt_data = float(loss)
            mt_acc = acc

        if not math.isnan(float(self.loss)):
            self.reporter.report(loss_st_data, loss_asr_data, loss_mt_data, st_acc, asr_acc, mt_acc)
        else:
