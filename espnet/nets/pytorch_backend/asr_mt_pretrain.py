...
class Reporter(chainer.Chain):
    ...
    def report(self, loss, acc_asr, acc_src, acc_trg, acc_mt):
        """Report at every step."""
        reporter.report({'loss': loss}, self)
        reporter.report({'acc_asr': acc_asr}, self)
        reporter.report({'acc_src': acc_src}, self)
...
class E2E(ASRInterface, torch.nn.Module):
    ...
    def __init__(self, idim, odim, args, ignore_id=-1, asr_model=None, mt_model=None):
        ...
        self.trg_predict = torch.nn.Linear(args.adim, 4997, bias=False)
        self.sos = odim - 1
        self.eos = odim - 1
        self.sos1 = 4996
        self.eos1 = 4996
        ...
        self.asr_criterion = LabelSmoothingLoss(self.odim, self.ignore_id, args.lsm_weight,
                                            False)                                 
        self.mt_criterion = LabelSmoothingLoss(4997, self.ignore_id, args.lsm_weight,
                                            False)
    ...
    def forward(self, xs_pad, ilens, ys_pad_src, ys_pad_trg, true_dist_src, true_dist_trg, starts, ends, ys_pad_asr=None, task="asr"):
        """E2E forward.
        ...
        if task == "asr":
            ...
            acc_asr = th_accuracy(pred_pad.view(-1, self.odim), ys_out_pad, ignore_label=-1)
        ...
        if task == "mt":
            xs_pad = xs_pad[:, :max(ilens)]  # for data parallel
            src_mask = (~make_pad_mask(ilens.tolist())).to(xs_pad.device).unsqueeze(-2)
            hs_pad, hs_mask = self.mt_encoder(xs_pad, src_mask)
            self.hs_pad = hs_pad
            ys_in_pad, ys_out_pad = add_sos_eos(ys_pad_src, self.sos1, self.eos1, self.ignore_id)
            ys_mask = target_mask(ys_in_pad, self.ignore_id)
            pred_pad, pred_mask = self.mt_decoder(ys_in_pad, ys_mask, hs_pad, hs_mask)

            # 3. compute attention loss
            loss_att = self.mt_criterion(pred_pad, ys_out_pad)
            acc_mt = th_accuracy(pred_pad.view(-1, 4997), ys_out_pad, ignore_label=-1)
            self.loss = loss_att
            acc_asr = None
            acc_src = None
            acc_trg = None
        ...
        if loss_data < CTC_LOSS_THRESHOLD and not math.isnan(loss_data):
            self.reporter.report(loss_data, acc_asr, acc_src, acc_trg, acc_mt)
        else:
