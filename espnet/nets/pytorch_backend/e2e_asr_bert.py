...
class E2E(STInterface, torch.nn.Module):
    ...
    def forward(self, xs_pad, ilens, ys_pad, transcript_label=None, task='tp'):
        """E2E forward.
        ...
        if task == "st":
            ...
            position_ids = torch.arange(seq_len, dtype=torch.long, device=hs_pad.device)
            ...
            segment_embeddings = self.bert.bert.embeddings.token_type_embeddings(segment_ids)
            hs_pad = hs_pad + position_embeddings + segment_embeddings
            enc_outputs = self.bert.bert.encoder(hs_pad, bert_mask, head_mask = head_mask)
            hs_pad = self.t2ddense(enc_outputs[0])
            # decoder forward
            loss, acc = self.decoder_forward(ys_pad,  hs_pad, hs_mask)
                
        if task == "tp":
            sp_hs_pad, sp_hs_mask = self.speech_encoder_forward(xs_pad, ilens)
            sp_hs_pad = self.s2tdense(sp_hs_pad)
            cls_token = torch.ones([sp_hs_pad.size(0), 1]).fill_(101).long().to(sp_hs_pad.device)
            cls_embedding = self.bert.bert.embeddings.word_embeddings(cls_token)
            sp_hs_pad = torch.cat([cls_embedding, sp_hs_pad], dim=1)
            sp_position_ids = torch.arange(sp_hs_pad.size(1), dtype=torch.long, device=sp_hs_pad.device)
            sp_token_ids = torch.zeros_like(sp_position_ids)
            #sp_position_embeddings = self.bert.bert.embeddings.position_embeddings(sp_position_ids)
            sp_token_embeddings = self.bert.bert.embeddings.token_type_embeddings(sp_token_ids)
            sp_hs_pad = sp_hs_pad + sp_token_embeddings
            cls_mask = torch.ones([sp_hs_pad.size(0), 1, 1], dtype=torch.uint8).to(sp_hs_mask.device)
            sp_hs_mask = torch.cat([cls_mask, sp_hs_mask], dim=-1)
            sp_bert_mask = self.get_bert_mask(sp_hs_mask)

            y_lens = [len(y[y!=self.ignore_id]) for y in ys_pad]
            ys_pad = ys_pad[:, :max(y_lens)]    #for data parallel
            txt_position_ids = torch.arange(ys_pad.size(1), dtype=torch.long, device=ys_pad.device)
            txt_token_ids = torch.ones_like(txt_position_ids) 
            txt_embed = self.bert.bert.embeddings(ys_pad, position_ids=txt_position_ids, token_type_ids=txt_token_ids)
            txt_mask = (~make_pad_mask(y_lens)).to(ys_pad.device).unsqueeze(-2)
            txt_bert_mask = self.get_bert_mask(txt_mask)

            hs_pad = torch.cat([sp_hs_pad, txt_embed], dim=1)
            bert_mask = torch.cat([sp_bert_mask, txt_bert_mask], dim=-1)
            head_mask = [None] * self.bert.config.num_hidden_layers
            enc_output = self.bert.bert.encoder(hs_pad, bert_mask, head_mask = head_mask)
            pooled_output = self.bert.bert.pooler(enc_output[0])
            seq_relationship_score = self.bert.cls(pooled_output)
            loss_fct = CrossEntropyLoss(ignore_index=-1)
            loss = loss_fct(seq_relationship_score.view(-1, 2),transcript_label.view(-1)) 
            pred = torch.argmax(seq_relationship_score, dim=-1)
            acc = pred == transcript_label
            acc = float(sum(acc)) / len(acc)
      
        if task == "mt":
            enc_outputs = self.bert(xs_pad)
            hs_pad = self.t2ddense(enc_outputs[0])
            pred_pad, pred_mask = self.decoder(ys_pad, hs_pad, hs_mask)
