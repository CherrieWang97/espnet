...
from espnet.nets.pytorch_backend.e2e_multitask import Reporter
...
class E2E(ASRInterface, torch.nn.Module):
    ...
    def __init__(self, idim, src_vocab_size, trg_vocab_size, args, asr_model=None, mt_model=None, st_model=None):
        ...
        args.transformer_attn_dropout_rate = 0
        self.senc = Encoder(
            idim=idim,
        ...
        # self.lsm_weight = a
        self.asr_criterion = LabelSmoothingLoss(self.src_vocab_size, self.ignore_id, args.lsm_weight,
                                            args.transformer_length_normalized_loss)
        self.criterion = LabelSmoothingLoss(self.trg_vocab_size, self.ignore_id, args.lsm_weight,
                                            args.transformer_length_normalized_loss)
        ...
        self.adim = args.adim
        self.ctc = CTC(src_vocab_size, args.adim, args.dropout_rate, ctc_type=args.ctc_type, reduce=True, bias=False)
        self.ctc.ctc_lo.weight = self.tenc.embed[0].weight
    ...
    def add_sos_eos(self, ys_pad, task="st"):
        from espnet.nets.pytorch_backend.nets_utils import pad_list
        if task == "st" or task == "mt":
    ...
    def forward(self, xs_pad, ilens, ys_pad, task="st"):
        ...
        if task == "st" or task == "asr":
            ...
            hs_pad, hs_mask = self.senc(xs_pad, src_mask)
            self.hs_pad = hs_pad
        elif task == "mt":
            xs_pad = xs_pad[:, :max(ilens)]  # for data parallel
            src_mask = (~make_pad_mask(ilens.tolist())).to(xs_pad.device).unsqueeze(-2)
            hs_pad, hs_mask = self.tenc(xs_pad, src_mask)
            self.hs_pad = hs_pad

        # forward decoder
        if task == "st" or task == "mt":
            ys_in_pad, ys_out_pad = self.add_sos_eos(ys_pad, task)
            ys_mask = self.target_mask(ys_in_pad)
            pred_pad, pred_mask = self.trgdec(ys_in_pad, ys_mask, hs_pad, hs_mask)
            self.pred_pad = pred_pad
        elif task == "asr":
            ys_in_pad, ys_out_pad = self.add_sos_eos(ys_pad, task)
            ys_mask = self.target_mask(ys_in_pad)
            pred_pad, pred_mask = self.srcdec(ys_in_pad, ys_mask, hs_pad, hs_mask)
            self.pred_pad = pred_pad
        # compute loss
        if task == "asr": 
            loss_att = self.asr_criterion(pred_pad, ys_out_pad)
            self.acc = th_accuracy(pred_pad.view(-1, self.src_vocab_size), ys_out_pad,
                               ignore_label=self.ignore_id)
        else:
            loss_att = self.criterion(pred_pad, ys_out_pad)
            self.acc = th_accuracy(pred_pad.view(-1, self.trg_vocab_size), ys_out_pad,
                               ignore_label=self.ignore_id)
        ...
        if task == "st":
            ...
            self.stacc = float(self.acc)
        elif task == "asr":
            self.loss = 0.5 * loss_ctc + 0.5 * loss_att
            self.asrloss = float(self.loss)
            self.asracc = float(self.acc)
        else:
            self.loss = loss_att
            self.mtloss = float(self.loss)
        loss_data = float(self.loss)
        if loss_data < CTC_LOSS_THRESHOLD and not math.isnan(loss_data):
            self.reporter.report(self.stloss, self.stacc, self.asrloss, self.asracc, self.mtloss)
        else:
