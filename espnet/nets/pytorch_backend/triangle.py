...
from espnet.nets.pytorch_backend.rnn.tridec import Decoder as triDecoder
from espnet.nets.pytorch_backend.rnn.multidec import Decoder as Decoder
...
class E2E(ASRInterface, torch.nn.Module):
    ...
    def __init__(self, idim, vocab_size, args, asr_model=None, mt_model=None, st_model=None):
        ...
        self.trgdec = triDecoder(args.eprojs, vocab_size, args.dtype, args.dlayers, args.dunits, self.sos, self.eos, self.trgatt,
                               args.verbose,
                                args.char_list, labeldist,
                                args.lsm_weight, args.sampling_probability, args.dropout_rate_decoder,
    ...
    def forward(self, xs_pad, ilens, ys_pad1, ys_pad2):
        ...
        zs = self.linear(zs)
        stloss, stacc, _, _ = self.trgdec(hs_pad, hlens, zs, zlens, ys_pad2, tgt_lang_ids=tgt_lang_ids)
        self.stacc = stacc
    ...
    def recognize(self, x, recog_args, char_list, rnnlm=None):
        ...
        y = self.srcdec.recognize_beam(hs[0], lpz, recog_args, recog_args.src_lang, char_list, rnnlm, strm_idx=1)
        pdb.set_trace()
        zs = torch.stack(y[0]['hidden'], dim=1)
        zs = self.linear(zs)
        y2 = self.trgdec.recognize_beam(hs[0], zs[0], lpz, recog_args, char_list, rnnlm)
