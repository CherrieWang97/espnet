...
mt_model=/hdfs/resrchvc/v-chengw/iwslt18/exp4mt/mt_wmt_share/results/model.acc.best
...
if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
    ...
    ${cuda_cmd} --gpu ${ngpu} exp/${expname}.log \
        ...
        --verbose ${verbose} \
        --resume ${resume} \
        --train-json ${feat_tr_dir}/share_comb.json \
        --valid-json ${feat_dt_dir}/share_comb.json \
        --asr-model ${asr_model} \
        --mt-model ${mt_model} 
fi
...
if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ]; then
    ...
    (
        ...
        if [ ${rtask} = "dev.de" ] || [ ${rtask} = "test.de" ]; then
            ...
            local/score_bleu_reseg.sh --bpemodel /hdfs/resrchvc/v-chengw/iwslt18/data4mt/dict/ted_share.model ${expdir}/${decode_dir} ${dict} ${st_ted} ${set}
        fi
    ) &
