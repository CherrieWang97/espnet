...
if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ]; then
    ...
    (
        ...
        ${decode_cmd} JOB=13:16 ${expdir}/${decode_dir}/log/decode.JOB.log \
            CUDA_VISIBLE_DEVICES=3 st_trans.py \
        ...
        local/score_bleu_reseg.sh --case ${case} --bpe ${nbpe} --bpemodel ${bpemodel}.model \
            ${expdir}/${decode_dir} ${dict} ${st_ted} tst2013
    ) &
