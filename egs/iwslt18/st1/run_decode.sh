...
resume=/hdfs/resrchvc/v-chengw/iwslt18/exp4st/st_transformer_char/results/snapshot.ep.4        # Resume the training from snapshot
seed=1          # seed to generate random number
# feature configuration
do_delta=false

train_config=conf/trans.yaml
...
expdir=/hdfs/resrchvc/v-chengw/iwslt18/exp4st/st_transformer_char
mkdir -p ${expdir}

if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
    echo "stage 4: Network Training"

    ${cuda_cmd} --gpu ${ngpu} exp/train-4.log \
        asr_train.py \
        --config ${train_config} \
        --ngpu ${ngpu} \
        --backend ${backend} \
        --outdir ${expdir}/results \
        --tensorboard-dir /hdfs/resrchvc/v-chengw/iwslt18/tensorboard/st_transformer_char \
        --debugmode ${debugmode} \
        --dict ${dict} \
        --debugdir ${expdir} \
        --minibatches ${N} \
